...
class Reserva {
    ...
    public function listarPorFeirante($feirante_id, $status_filter = []) {
        $query = "SELECT r.id, u.nome as cliente_nome, o.nome as oferta_nome, r.quantidade_reservada, r.codigo_retirada, r.status, r.data_reserva FROM " . $this->table_name . " r JOIN usuarios u ON r.consumidor_id = u.id JOIN ofertas o ON r.oferta_id = o.id WHERE o.feirante_id = :feirante_id";
    ...
    public function listarPorConsumidor($consumidor_id, $status_filter = []) {
        $query = "SELECT r.id, r.codigo_retirada, r.quantidade_reservada, r.status, DATE_FORMAT(r.data_reserva, '%d/%m/%Y às %H:%i') as data_reserva_formatada, o.nome as oferta_nome, o.foto, u.nome as feirante_nome, (r.preco * r.quantidade_reservada) as valor_total FROM " . $this->table_name . " r JOIN ofertas o ON r.oferta_id = o.id JOIN usuarios u ON o.feirante_id = u.id WHERE r.consumidor_id = :consumidor_id";
    ...
    public function buscarPorId($reserva_id) {
        $query = "SELECT r.id, r.codigo_retirada, r.status, o.nome as oferta_nome, u.nome as feirante_nome FROM " . $this->table_name . " r JOIN ofertas o ON r.oferta_id = o.id JOIN usuarios u ON o.feirante_id = u.id WHERE r.id = :id LIMIT 1";
        $stmt = $this->conn->prepare($query);
    ...
    /**
     * Busca os detalhes de uma reserva pelo seu código de retirada.
     */
    public function buscarPorCodigoRetirada($codigo_retirada) {
        $query = "SELECT 
                        r.id,
                    ...
                    WHERE 
                        r.codigo_retirada = :codigo_retirada
                    LIMIT 1";

        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':codigo_retirada', $codigo_retirada, PDO::PARAM_STR);
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }
}
